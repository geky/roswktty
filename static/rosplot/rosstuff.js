...
var ROSPlot = (function() {
  ...
  var cmds = {
    ...
    show: {
      help: 'renders quantities of data',
      ...
      cmd: function(input) {
        ...
        for (var i = 1; i < input.length; i++) {
          ...
          !function(topic) {
            topic.refs[0] = ros.subdata(topic.name, function(data) {
              topic.values[0] = data;
            }, function(newname) {
              var i = topic.values.length;

              topic.refs.push(ros.subscribe(newname, function(data) {
                topic.values[i] = data;
              }));
            }, function() {
              ros.unsubscribe(topic.shift());
            });
          }(topic);
        ...
        this.plot(function(ctx) {
          ...
          for (var i = 0; i < topics.length; i++) {
            for (var ii = 0; ii < topics[i].values.length; ii++) {
              ...
              if (topics[i].values[ii] < min) {
                min = topics[i].values[ii];
              }
            }
          }
        ...
        this.defer(function() {
          for (var i = 0; i < topics.length; i++) {
            for (var ii = 0; ii < topics[i].refs.length; ii++) {
                ros.unsubscribe(topics[i].refs[ii]);
            }
          }
        });
      }
